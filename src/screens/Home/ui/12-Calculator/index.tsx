...
import { roundToStep } from "@/shared/lib/roundToStep";
...
const config = {
  gp: [
    { name: "Management Fees", value: 3.6 }, // "$3.6M"
    { name: "Carry (Catch-up + Split)", value: 7.8 }, // "$7.8M"
    { name: "Total GP Earnings", value: 7.8 }, // "$7.8M"
  ],
  lp: [
    { name: "Capital Returned", value: 30.0 }, // "$30.0M"
    { name: "Preferred Return", value: 17.6 }, // "$17.6M"
    { name: "Profit Share", value: 14.0 }, // $14.0M"
    { name: "Total LP Return", value: 61.6 }, // "$61.6M"
  ],
  summary: [
    { name: "Terminal Gross Value", value: 69.4 }, // "$69.4M"
    { name: "Management Fees", value: 3.6 }, // "$3.6M"
    { name: "GP Carry", value: 4.2 }, // "$4.2M"
    { name: "LP Total", value: 61.6 }, // "$61.6M"
  ],
};
      ...
      <div className="mx-auto max-w-380 w-full">
        ...
        >
          ...
          >
            ...
            <div className="" ref={triggerRef}>
              ...
              <div className="mt-6 grid grid-cols-2 gap-x-6 gap-y-8 grid-flow-dense max-sm:grid-cols-1 max-sm:gap-4.5">
                <InputRange
                  ...
                  renderDisplayValue={(val) => `${roundToStep(val, 1)}M`}
                  displayMin="5M"
                  displayMax="500M"
                  title="Commitments Raised (Day 0)"
                  rangeAnimation
                  isAnimationStart={isAnimation}
                />
                <InputRange
                  min={0}
                  max={12}
                  ...
                  renderDisplayValue={(val) => `${roundToStep(val, 1)}%`}
                  displayMin="0%"
                  displayMax="12%"
            ...
            {/* Right */}
            <ContentCard
              className="calc-aos flex flex-col rounded-4xl p-7.5 py-6.5 max-sm:rounded-2xl max-sm:px-4 max-sm:py-4.5"
                    ...
                    {config.gp.map((item) => (
                      ...
                      >
                        ...
                        <p className="value text-right font-tthoves text-2xl leading-none -tracking-1 text-primary group-last:text-[#9C77FF]/80 max-xxl:text-18 max-sm:text-xs">
                          $<CountUp value={item.value} delay={0.6} />M
                        </p>
                      </li>
                    ))}
                  </ul>
                    ...
                    {config.lp.map((item) => (
                      ...
                      >
                        ...
                        <p className="value text-right font-tthoves text-2xl leading-none -tracking-1 text-primary group-last:text-[#6776FF]/80 max-xxl:text-18 max-sm:text-xs">
                          $<CountUp value={item.value} delay={0.8} />M
                        </p>
                      </li>
                    ))}
                  </ul>
                    ...
                    {config.summary.map((item) => (
                      ...
                      >
                        ...
                        <p className="font-tthoves text-32 leading-tight -tracking-1 text-primary max-xxl:text-20">
                          $<CountUp value={item.value} delay={1} />M
                        </p>
                      </ContentCard>
                    ))}
                  </ul>
